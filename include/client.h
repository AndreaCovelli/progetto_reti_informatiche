...
/**
 * Gestisce la risposta del server alla richiesta di nickname
 * @param msg puntatore al messaggio ricevuto dal server
 * @return true se il nickname è stato accettato, false altrimenti
 * @note Interpreta la risposta del server e mostra messaggi appropriati
 */
bool handle_nickname_response(Message* msg);

/**
 * Invia una richiesta di nickname al server
 * @param state struttura ClientState
 * @return true se l'invio ha avuto successo, false altrimenti
 * @note Prepara e invia un messaggio di tipo MSG_REQUEST_NICKNAME
 */
